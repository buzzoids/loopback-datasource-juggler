...
function coerceArray(val) {
  ...
  for (var i = 0; i < arrayVal.length; ++i) {
    ...
    console.log ('COERCE ARRAY', val[i]);
    arrayVal[i] = val[i];
  }
  ...
  for (var p in where) {
    ...
    if (!DataType) {
      ...
      if (p.indexOf('.') !== -1) {
        ...
        // NOTE: Do not loop, because it affects search on Arrays with nested properties

          let part = (_.get(obj, p, undefined));

          DataType = (_.get(part, 'type', false) === 'date' ? Date : undefined);
    ...
    //console.log ('p in where - 3', DataType);
    if (!DataType) {
      continue;
    }
        ...
        if (op in val) {
          ...
          switch (operator) {
            ...
            case 'between':
              ...
              try {
                val = coerceArray(val);
                console.log ('inside between 1', val);
              } catch (e) {
                err = new Error(g.f('The %s property has invalid clause %j: %s', p, where[p], e));
                err.statusCode = 400;
                throw err;
